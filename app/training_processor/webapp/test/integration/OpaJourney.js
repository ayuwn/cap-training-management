...
sap.ui.define(["sap/ui/test/opaQunit"], function (opaTest) {
  ...
  const Journey = {
    start: function () {
      QUnit.module("Training Processor Tests");
    ...
    test: function () {
      ...
      opaTest(
        ...
        function (Given, When, Then) {
          When.onTheMainPage.onTable().iPressRow({ Training: "4,133" });
      ...
      opaTest("#3: List report: Create training", function (Given, When, Then) {
        Then.onTheMainPage.iSeeThisPage();
        ...
        When.onTheDetailPage
          .onForm({ section: "TrainingRequest", fieldGroup: "TrainingData" })
          .iOpenValueHelp({ property: "to_Training_TrainingID" });
        When.onTheDetailPage.onValueHelpDialog().iSelectRows({ 0: "070006" });

        // Value help Customer ID
        When.onTheDetailPage
          .onForm({ section: "TrainingRequest", fieldGroup: "TrainingData" })
          .iOpenValueHelp({ property: "to_Employee_EmployeeID" });
        ...
        When.onTheDetailPage
          .onForm({ section: "TrainingRequest", fieldGroup: "DateData" })
          .iChangeField(
            { property: "BeginDate" },
        ...
        // Currency
        When.onTheDetailPage
          .onForm({ section: "TrainingRequest", fieldGroup: "PriceData" })
          .iChangeField({ property: "CurrencyCode_code" }, "EUR");

        // Description
        When.onTheDetailPage
          .onForm({ section: "TrainingRequest", fieldGroup: "TrainingData" })
          .iChangeField({ property: "Description" }, "Training for deletion");
      ...
      opaTest("#4: List report: Delete training", function (Given, When, Then) {
        Then.onTheMainPage.iSeeThisPage();
      ...
      opaTest(
        ...
        function (Given, When, Then) {
          ...
          Then.onTheMainPage
            .onTable()
            .iCheckAction(
              { service: "TrainingService", action: "acceptTrainingRequest" },
              { visible: true, enabled: false }
            );

          // select first row
          Given.onTheMainPage.onTable().iSelectRows({ Training: "4,132" });
          ...
          Then.onTheMainPage
            .onTable()
            .iCheckRows({ Travel: "4,132", "Training Status": "Open" }, 1);
